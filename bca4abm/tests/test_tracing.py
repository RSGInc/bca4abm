...
from .. from activitysim.core import tracing as tracing


def close_handlers():
    for logger_name in ['bca4abm', 'orca']:
...
def test_bad_custom_config_file(capsys):
    ...
    bca_logger_baseFilename = file_handlers[0].baseFilename
    ...
    with open(bca_logger_baseFilename, 'r') as content_file:
