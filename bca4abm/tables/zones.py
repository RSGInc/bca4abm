...
def zone_cvals(data_dir, settings):
    ...
    base_cocs_df = read_csv_file(data_dir=os.path.join(data_dir, 'base-data'),
        file_name=cocs_file_names)

    base_cvals_df = pd.concat([base_cvals_df, base_cocs_df], axis=1)

    build_cocs_df = read_csv_file(data_dir=os.path.join(data_dir, 'build-data'),
        file_name=cocs_file_names)
