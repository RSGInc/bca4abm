...
def aggregate_od_processor(zone_demographics, aggregate_od_spec, settings, data_dir, trace_od):
    ...
    results, trace_results, trace_assigned_locals = \
        bca.eval_and_sum(aggregate_od_spec,
                         od_df,
                         locals_dict,
                         df_alias='od',
                         chunk_size=0,
    ...
    # convert series of results indexed by target to dataframe with target as column name
    results = pd.DataFrame(data=results).T

    # FIXME - just add summary results? isn't this supposed to be broken down by coc?

    print "\n\naggregate_od_processor - What are we supposed to be writing as results?"
    print "                         Shouldn't this be broken down by coc?\n\n"

    add_summary_results(results, prefix='OD_', spec=aggregate_od_spec)
